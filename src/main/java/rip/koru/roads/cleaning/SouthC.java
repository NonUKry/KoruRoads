...
public class SouthC {
    ...
    public static void start(Player player) {
        ...
        Bukkit.broadcastMessage(CC.GREEN + "Starting the road SOUTH cleaning!");
        ...
        for(int y = 0; y < 250; y++) {
            ...
            Bukkit.broadcastMessage(CC.GREEN + "Cleaning the road SOUTH (" + new DecimalFormat("##.##").format((value)) + "%)");
        }
        if(value != 100.00D) {
            Bukkit.broadcastMessage(CC.GREEN + "Cleaning the road SOUTH (100%)");
        }
        Bukkit.broadcastMessage(CC.GREEN + "The SOUTH road has been successfully cleared in " + (double) TimeUnit.MILLISECONDS.toSeconds(System.currentTimeMillis() - initTime) + " secconds!");
    }
}
